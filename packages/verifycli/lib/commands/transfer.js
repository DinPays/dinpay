...
const primaryAddress = dinpay.crypto.getAddress(dinpay.crypto.getKeys(config.passphrase).publicKey)
...
module.exports = async (options, wallets, DinPayPerTx, skipTestingAgain) => {
  ...
  let transactionAmount = (DinPayPerTx || 2) * Math.pow(10, 8)
  if (options.amount) transactionAmount = options.amount

  wallets.forEach((wallet, i) => {
    const transaction = dinpay.transaction.createTransaction(
