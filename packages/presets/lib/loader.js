...
class ConfigLoader {
  ...
  async setUp (options) {
    ...
    this.network = JSON.parse(process.env.DINPAY_LEDGER)
  ...
  async copyFiles (dest) {
    if (!dest) {
      dest = `${process.env.DINPAY_DATA_PATH}/config`
    }

    await fs.ensureDir(dest)

    return fs.copy(process.env.DINPAY_PRESET_PATH, dest)
  }
   ...
   */
  __getFiles () {
    const basePath = path.resolve(process.env.DINPAY_PRESET_PATH)
