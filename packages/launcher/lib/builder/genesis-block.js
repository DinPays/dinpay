const dinpayjs = require('dinpayjs')
...
module.exports = class GenesisBlockBuilder {
  ...
  constructor (network, options) {
    ...
    dinpayjs.crypto.setNetworkVersion(this.prefixHash)
  }
  ...
  __createWallet () {
    ...
    const keys = dinpayjs.crypto.getKeys(passphrase, {network: this.network})

    return {
      address: dinpayjs.crypto.getAddress(keys.publicKey, this.prefixHash),
      passphrase,
      keys
    }
  }
  ...
  __createTransferTransaction (senderWallet, receiverWallet, amount) {
    return this.__formatGenesisTransaction(
      dinpayjs.transaction.createTransaction(receiverWallet.address, amount, null, senderWallet.passphrase, undefined, this.prefixHash),
      senderWallet
    )
  }
  ...
  __createDelegateTransaction (wallet) {
    return this.__formatGenesisTransaction(
      dinpayjs.delegate.createDelegate(wallet.passphrase, wallet.username),
      wallet
    )
  }
  ...
  __formatGenesisTransaction (transaction, wallet) {
    ...
    transaction.signature = dinpayjs.crypto.sign(transaction, wallet.keys)
    transaction.id = dinpayjs.crypto.getId(transaction)
  ...
  __createGenesisBlock (data) {
    ...
    transactions.forEach(transaction => {
      const bytes = dinpayjs.crypto.getBytes(transaction)
