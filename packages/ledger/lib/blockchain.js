...
module.exports = class Blockchain {
  ...
  constructor (config, networkStart) {
    ...
    if (stateMachine.state.networkStart) {
      logger.warn('DinPay is launched in Genesis Start mode. This is usually for starting the first node on the blockchain. Unless you know what you are doing, this is likely wrong.')
      logger.info('Starting DinPay for a new world, welcome aboard :rocket:')
    }
  ...
  async start (skipStartedCheck = false) {
    ...
    if (skipStartedCheck || process.env.DINPAY_SKIP_BLOCKCHAIN_STARTED_CHECK) {
      return true
    }

    // TODO: this state needs to be set after the state.lastBlock is available if DIN_ECOSYSTEM=test
   ...
   */
  get p2p () {
    return dpCapsule.resolvePlugin('p2p')
  }
   ...
   */
  get transactionPool () {
    return dpCapsule.resolvePlugin('transactionPool')
  }
   ...
   */
  get database () {
    return dpCapsule.resolvePlugin('database')
  }
