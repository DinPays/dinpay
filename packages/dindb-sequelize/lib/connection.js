...
const { ConnectionInterface } = require('@dinpay/dindb')
...
const emitter = dpCapsule.resolvePlugin('event-emitter')

const { Block, Transaction } = require('@dinpay/encryption').models
...
module.exports = class SequelizeConnection extends ConnectionInterface {
  ...
  async buildWallets (height) {
    ...
    const spvPath = `${process.env.DINPAY_DATA_PATH}/spv.json`
    if (fs.existsSync(spvPath)) {
      fs.removeSync(spvPath)
      logger.info('DinPay ended unexpectedly - resuming from where we left off :runner:')
