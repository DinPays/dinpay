...
const transactionPool = dpCapsule.resolvePlugin('transactionPool')
...
exports.getHeight = {
   ...
   */
  handler (request, h) {
    const lastBlock = dpCapsule.resolvePlugin('blockchain').getLastBlock(true)
...
exports.getTransactionsFromIds = {
  ...
  async handler (request, h) {
    try {
      ...
      const transactions = await dpCapsule.resolvePlugin('database').getTransactionsFromIds(transactionIds)
...
exports.getBlocks = {
  ...
  async handler (request, h) {
    try {
      const blocks = await dpCapsule.resolvePlugin('database').getBlocks(parseInt(request.query.lastBlockHeight) + 1, 400)
