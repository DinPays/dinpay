...
module.exports = class Monitor {
  ...
  async updateNetworkStatus () {
    try {
      ...
      if (process.env.DIN_ECOSYSTEM !== 'test') {
        await this.discoverPeers()
        await this.cleanPeers()
      }

      if (Object.keys(this.peers).length < this.config.peers.list.length - 1 && process.env.DIN_ECOSYSTEM !== 'test') {
        this.config.peers.list
  ...
  async acceptNewPeer (peer) {
    if (this.peers[peer.ip] || process.env.DIN_ECOSYSTEM === 'test') {
      return
    }
